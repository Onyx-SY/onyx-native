...
def get_lang_text(lang: str) -> Dict[str, str]:
    if lang == "english":
        return {
            ...
            "plan_mode_warning": "⚠️ You are currently in PLAN mode. You can ONLY generate plans — you MUST NOT execute any commands or modify any files. Submit your plan via the submit_plan tool. Wait for user confirmation before execution.",
            ...
            # ── 交互式会话新命令 ──
            "quiet_on": "🔇 Quiet mode ON (suppressing auxiliary info)",
            "quiet_off": "🔊 Quiet mode OFF",
            "no_token_data": "No token data yet — start a conversation first",
            "reset_done": "✅ Conversation context reset, starting fresh",
            "export_ok": "✅ Conversation exported to: {}",
            "export_fail": "❌ Export failed: {}",
            "export_no_data": "No conversation data to export",
            "lang_invalid": "Invalid language: {}. Use cn or en",
            "lang_switched": "🌐 Language switched",
            "mode_switched": "🔄 Mode switched to: {}",
            "mode_invalid": "Invalid mode: {}. Use normal or plan",
            "time_on": "⏱️ Timing display ON",
    ...
    return {
            ...
            "task_complete": "✅ 工作完成！",
            "plan_mode_warning": "⚠️ 当前处于 PLAN 模式。你只能生成计划 — 不能执行任何命令或修改文件。请通过 submit_plan 工具提交你的计划。等用户确认后，才能进入执行阶段。",
            ...
            # ── 交互式会话新命令 ──
            "quiet_on": "🔇 精简模式已开启（隐藏辅助信息）",
            "quiet_off": "🔊 精简模式已关闭",
            "no_token_data": "暂无 token 数据 — 请先开始对话",
            "reset_done": "✅ 对话上下文已重置，开始新对话",
            "export_ok": "✅ 对话已导出到：{}",
            "export_fail": "❌ 导出失败：{}",
            "export_no_data": "没有可导出的对话数据",
            "lang_invalid": "无效的语言：{}。使用 cn 或 en",
            "lang_switched": "🌐 语言已切换",
            "mode_switched": "🔄 模式已切换为：{}",
            "mode_invalid": "无效模式：{}。使用 normal 或 plan",
            "time_on": "⏱️ 耗时显示已开启",
            "time_off": "⏱️ 耗时显示已关闭",
        }
