...
def _restore_cwd(saved: Optional[str]) -> None:
    """恢复进程 cwd 到会话快照，并同步持久 shell / 失效 CWD 缓存。"""
    try:
        deactivate()  # 无论会话以何种方式结束（含异常），停用 AI 沙盒
    except Exception:
        pass
    if not saved:
        return
...
def session_guard():
    """AI 会话 cwd 守卫（2026-09 修复）：进入 AI 前快照 cwd，会话结束恢复。

    内置 cd 在 Onyx 进程内直接 os.chdir —— AI 会话内执行 cd（如 cd /）会把
    主 shell 的工作目录一起带走，退出 AI 后相对路径 cd 全失效、cd .. 被
    沙箱边界拦截。用 with 包住整个 AI 会话（含一次性调用与持久 REPL），
    退出（含异常）时自动恢复进程 cwd、同步持久 shell 并停用 AI 沙盒。
    """
    saved = None
    try:
        saved = os.getcwd()
...
def is_within(phys_path: str) -> bool:
    ...
    对路径做 realpath：符号链接会解析到真实位置再判定，
    封死「工作区内 symlink → 外部」的沙盒逃逸。
    """
    root = _root
    if not root:
        return True
    ...
    try:
        ...
        # 目标文件不存在时误判为普通路径，导致走 abspath 分支漏判）。
        _rp = os.path.realpath(phys_path)
    except Exception:
        try:
            _rp = os.path.abspath(phys_path)
        except Exception:
            _rp = phys_path
    _rp = os.path.normpath(_rp)
    return _rp == root or _rp.startswith(root + os.sep)
...
def resolve(vpath: str) -> str:
    ...
    # 绝对路径（/x）→ 虚拟根映射；已在沙箱内的物理路径 → 原样返回。
    # 不存在于两处的路径也一律映射进沙盒（原「原样放行」分支已删除：
    # 它会允许向真实文件系统根写文件，构成沙盒逃逸）。
    elif vpath.startswith("/"):
        if is_within(vpath):
