...
def get_chat_json_path(home_dir: str, chat_name: str) -> str:
    ...
    # 2026-09 加固（M7）：chat 名只允许单段文件名——路径分隔符/.. 会经
    # f"{chat_name}.json" 拼接逃出 chat 目录（任意位置创建/覆盖 .json）。
    # 统一做无害化替换，create/switch/load 都经过本函数，行为一致。
    _safe = str(chat_name or "").replace("/", "_").replace("\\", "_")
    if _safe in ("", ".", "..") or ".." in _safe:
        _safe = "_"
    return os.path.join(chat_dir, f"{_safe}.json")
...
def _safe_session_id(session_id: str) -> bool:
    """2026-09 加固（M2）：session/memory id 必须是单段文件名（UUID/chat 名），
    拒绝路径分隔符与 .. 穿越，防止拼接后读取/写入库目录之外的文件。"""
    if not session_id:
        return False
...
def get_latest_ai_session(home_dir: str, session_id: str) -> Tuple[str, str]:
    ...
    # 2026-09 加固：非法 session_id 直接返回空（拒绝路径穿越）
...
def load_memory_by_uuid(home_dir: str, memory_uuid: str) -> str:
    library_dir = get_ai_session_library_dir(home_dir)
    # 2026-09 加固：非法 uuid 直接返回空（拒绝路径穿越）
    if not _safe_session_id(memory_uuid):
        return ""
