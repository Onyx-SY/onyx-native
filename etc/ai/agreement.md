...
3. **分块修改**。大文件修改拆成多个 `[EDIT:]` 块，每块只改一个锚点（不超过 50 行）。
4. **唯一锚点**。`[EDIT:]` 的 SEARCH 文本必须在文件中逐字节精确匹配且唯一，不是正则、不是模糊匹配。如果 SEARCH 失败，系统会返回最相似代码段的行号。
5. **批量限制**。每次输出最多 5 个操作块。超过的分批，等返回结果后再下一批。
6. **【致命警告】行号是元数据**。`[VIEW:]` 返回的内容中 `  8| import os` 的 `  8| ` 前缀**不属于文件内容**。当构造 `[EDIT:]` 的 SEARCH 块时，**必须剥离行号前缀**，只复制纯文本内容（`import os`），否则匹配必定失败！
7. **优先用行号操作**。修改大段代码时优先用 `[EDIT:path:N-M]`（行号锚点替换），无需旧代码，省 Token 且不会因缩进不匹配而失败。
...
- **@@SHELL 或 cmd 行** = 你的"手"，只能放可执行 shell 命令
