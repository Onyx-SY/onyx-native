...
6. **禁止编辑二进制文件**。系统自动拦截 `.png`/`.jpg`/`.pdf`/`.zip`/`.exe` 等二进制文件的编辑操作，请使用 shell 命令处理。
7. **自动创建目录**。`[WRITE:path]` 会自动创建父目录，无需手动 `mkdir`。
8. **大文件用 WRITE**。修改超过 70% 代码时，推荐 `[VIEW:path]` 读完整文件 → `[WRITE:path]` 一次性写入。
