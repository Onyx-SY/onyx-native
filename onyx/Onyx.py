...
def validate_param_path(param: str, request_id: str) -> str:
    ...
    try:
        ...
        # 4. 安全决策已全部在 resolve_path 内完成，不再做二次沙箱校验：
        #    - 越界 → resolve_path 返回 FORBIDDEN_MSG（此处转空值使命令失败）
        #    - 虚拟路径 → 已映射到虚拟根内（安全）
        #    - 原样放行（/api、/dev/* 等）→ 假路径或特殊白名单，无威胁
        #    二次 check_sandbox_path 会把 /api 这类假路径误拦为「虚拟沙箱外的路径」。
        if resolved_path == FORBIDDEN_MSG:
            log_warning(f"参数{param}越界被拦截，返回空值使命令失败", request_id)
            return ""
...
def execute_tool(tool_info: ToolInfo, args: List[str], request_id: str) -> None:
    ...
    # 1. 工具权限校验：检查当前模式是否有权限调用
    perm_check = check_tool_permission(tool_info.tool_perm)
    if not perm_check:
        return

    # 3. 核心参数处理：按规则验证每个参数，非路径参数保留原始值
    #    路径安全已由 validate_param_path（内部走 resolve_path）保证，
    #    不再做二次沙箱校验（避免误拦 /api 等合法 API 端点）
    resolved_args = [validate_param_path(arg, request_id) for arg in args]
...
def initialize_onyx_environment(request_id: str, oneshot: bool = False) -> bool:
    ...
    try:
        ...
        # 2026-09：root 用户默认 mid —— root 的虚拟 home 是 /root，perm_path.json 的
        # /root/<*:10> 规则要求 mid 及以上；若仍默认 low，root 用户在自己主目录会被
        # min_mode 全面拦截（需 adv 才能操作 /root 路径）。仅在配置未显式改档时生效。
        if user_info.get("is_admin", False) and str(default_mode).lower() == "low":
            default_mode = "mid"
