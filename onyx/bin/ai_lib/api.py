...
# ── 当前活跃 HTTP 响应（按 session_id 隔离；Ctrl+C 只关主会话的响应）──
_ACTIVE_RESPONSES: Dict[str, Any] = {}


def abort_active_response(session_id: str = "") -> None:
    """关闭并移除指定会话的活跃 HTTP 响应（Ctrl+C 中断用）。"""
    _resp = _ACTIVE_RESPONSES.pop(session_id or "_default", None)
...
# 按 session_id 隔离：子代理（explore_xxx）与压缩摘要（compact_*）的请求在
# 后台线程/独立流程运行，若共用一个全局量，主会话下一轮
# _estimate_conversation_tokens 会拿子代理的字符数校准 tokPerChar → 估算虚高
# → AutoCompact 提前触发（历史中段重写 = 一次缓存断裂）。
_last_request_chars: Dict[str, int] = {}


def get_last_request_chars(session_id: str = "") -> int:
    """返回指定会话最近一次 API 请求的 messages 总字符数（不含工具 schema）。

    session_id 为空时回退默认槽位（兼容无会话隔离的调用方）。
    """
    return _last_request_chars.get(session_id or "_default", 0)


def clear_last_request_chars(session_id: str = "") -> None:
    """会话结束时清理字符数槽位，避免长期运行的进程无限增长。"""
    _last_request_chars.pop(session_id or "_default", None)


# 调试文件（ai_request.txt / ai_payload.json）写入锁：主会话与并发子代理/压缩
# 请求会同时写这两个固定路径文件，不加锁会产生半写损坏或互相覆盖。
...
{question}"""
    ...
    #   - anthropic 分支已转换为 Anthropic 格式（_convert_tools_for_anthropic），
    ...
    # 并发子代理/压缩请求与主会话同时写这两个固定路径文件，
    # 加锁串行化防止半写损坏；头部标注 session_id 便于区分来源。
    try:
        ...
        with _DEBUG_FILE_LOCK:
            ...
            _lines = [f"═══ API REQUEST ({plat_key}/{model}, session={session_id or '-'}) — {datetime.now().strftime('%H:%M:%S')} ═══", ""]
    ...
    # 中断标志不复位：复位权收归主循环（handle_ai 每轮 API 调用前显式复位）。
    # 若在这里复位，并发子代理/压缩线程的请求会把用户 Ctrl+C 置位的中断标志
    # 清零 → 主循环中断失效、继续烧 token；子代理也不应重置它（中断后应快速停止）。
    ...
    _slot = session_id or "_default"
    _last_request_chars[_slot] = 0
    try:
        for _m in _messages:
            _c = _m.get("content")
            if isinstance(_c, str):
                _last_request_chars[_slot] += len(_c)
            elif isinstance(_c, list):
                for _blk in _c:
                    if isinstance(_blk, dict):
                        _last_request_chars[_slot] += len(str(_blk.get("text", "")))
                        _last_request_chars[_slot] += len(str(_blk.get("input", "")))
            _rc = _m.get("reasoning_content")
            if isinstance(_rc, str):
                _last_request_chars[_slot] += len(_rc)
            for _tc in _m.get("tool_calls") or []:
                _args = _tc.get("function", {}).get("arguments", "")
                if isinstance(_args, str):
                    _last_request_chars[_slot] += len(_args)
    except Exception:
        pass
