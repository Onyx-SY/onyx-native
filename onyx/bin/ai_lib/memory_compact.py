...
def estimate_tokens(text: str) -> int:
    ...
    # 其余字符（标点、空格、换行等）：精确计数，不再从总长粗略扣除（长英文词曾导致负数）
    remaining = len(re.sub(r'[a-zA-Z_][a-zA-Z0-9_]*|[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]', '', text))
    
    # 加权估算：英文 ~1.3 tokens/词，中文 ~1.8 tokens/字，其余 ~0.25 tokens/字符
    est = int(eng_words * 1.3 + cjk_chars * 1.8 + remaining * 0.25)
...
def _llm_summarize_chunk(chunk_entries: List[Dict], user_home_dir: Optional[str],
    ...
    try:
        ...
        if not txt:
            return None
        # ── 成本记录：压缩 LLM 调用同样入账 cost.json（与子代理一致）──
        try:
            _usage = result.get("_usage") or {}
            _pt = _usage.get("prompt_tokens") or 0
            ...
            if _pt or _ct:
                ...
                append_cost_record(user_home_dir or os.path.expanduser("~"),
                                   _conf.get("platform", "deepseek"),
                                   _conf.get("model", ""), _pt, _ct)
        except Exception:
            pass
