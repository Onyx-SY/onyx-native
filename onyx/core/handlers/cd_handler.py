...
def handle_cd(ctx: "AppContext", cmd_parts: List[str], request_id: str) -> None:
    ...
    try:
        ...
        if not paths:
            ...
            # AI 会话内：/ → AI 沙盒根（会话启动时的 cwd），避免 cd / 跳到应用根目录
            try:
                from bin.ai_lib import sandbox as _ai_cd_sb
                if _ai_cd_sb.is_active():
                    target_path = _ai_cd_sb.get_root() or (ctx.ROOT_DIR if ctx._SANDBOX_ENABLED else "/")
                else:
