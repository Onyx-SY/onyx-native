...
def check_sandbox_path(ctx: "AppContext", path: str, request_id: str) -> bool:
    ...
    路径安全决策已全部由 lib/resolve_path 完成：
      - 越界路径 → 返回 FORBIDDEN_MSG 拦截
      - 虚拟路径 → 映射到虚拟根内
      - /api 等假路径 / 网络接口 → 原样放行
    此处二次校验会把 /api 这类合法 API 端点误拦为「虚拟沙箱外的路径」，
    故不再拦截。保留函数签名以兼容所有调用方（parse_and_execute、
    run_cmd、mktool、source、AI 工具链等）。
    """
    return True
