...
class ApprovalTokenLedger:
    ...
    def verify(self, token: str, scope: ApprovalScope,
        ...
        if grant.uses >= grant.max_uses:
            ...
            return False, "令牌已用完"
        # 作用域匹配（2026-09 加固 M6）：精确匹配，拒绝子串放行。
        # 原实现是 `scope.action not in grant.scope.action` 子串判断——
        # grant="write_file /etc/foo" 时请求 "write_file /etc" 即可通过，
        # 更宽泛的作用域请求可绕过单文件授权。现改为：完全相等，或 grant 为 "*"（全量授权）。
        if scope.action and grant.scope.action:
            if grant.scope.action != "*" and scope.action != grant.scope.action:
                return False, f"令牌作用域不匹配: {scope.action} != {grant.scope.action}"
        return True, "ok"
