...
int check_command(const char* cmd, const char* root_dir, const char* user_home, const char* protected_paths_str) {
    ...
    // 2026-09 修复（M8）：改用 strtok_r/strtok_s 分离保存指针。
    // 原实现对两个字符串交替调用 strtok（C11 单静态指针）——内层循环结束后，
    // 外层 strtok(NULL) 继续解析的是已被 free+重新 STRDUP 的 protected 缓冲，
    // 属未定义行为，多路径参数命令的后续路径不被正确校验。
    char* path_save = NULL;
    char* path_token = STRTOK_R(paths_copy, ",", &path_save);
    while (path_token && result) {
        ...
        char* prot_save = NULL;
        char* prot_token = STRTOK_R(protected_copy, ",", &prot_save);
        while (prot_token) {
            ...
            prot_token = STRTOK_R(NULL, ",", &prot_save);
        }
