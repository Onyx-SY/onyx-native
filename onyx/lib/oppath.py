...
def check_oppath(cmd: str) -> bool:
    ...
    # Python降级逻辑（与C库完全对齐）
    # 2026-09 加固（H5）：全程用 realpath 解析符号链接——原 abspath 实现下，
    # home 内 symlink 指向受保护路径（如 ln -s ../etc/pki/x.key link）时，
    # 未解析路径落在 home 前缀内被放行，shell 实际读取受保护文件。
    user_home, _ = _get_user_home_dir_python()
    user_home_abs = os.path.realpath(user_home).lower().replace("\\", "/")
    ...
    for cmd_path in cmd_paths:
        cmd_path_abs = os.path.realpath(cmd_path).lower().replace("\\", "/")
        
        # 主目录内路径 → 放行
        if cmd_path_abs.startswith(user_home_abs):
            continue
        
        # 保护路径校验（保护项同样 realpath，保证比较一致）
        for protected in protected_paths:
            protected = os.path.realpath(protected).replace("\\", "/")
