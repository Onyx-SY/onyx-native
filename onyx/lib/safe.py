...
# === 会话级验证码确认（2026-09）：同一会话内验证过一次后，后续命令执行不再重复弹验证码 ===
_SESSION_CAPTCHA_VERIFIED = False  # 会话内是否已完成过一次验证码确认


def is_session_captcha_verified() -> bool:
    """本会话是否已完成过验证码确认（进程生命周期内有效）"""
    return _SESSION_CAPTCHA_VERIFIED


def mark_session_captcha_verified() -> None:
    """标记本会话已完成验证码确认（验证码正确输入一次后置位）"""
    global _SESSION_CAPTCHA_VERIFIED
    _SESSION_CAPTCHA_VERIFIED = True
...
def _mode_below(current_mode: str, required_mode: str) -> bool:
    """当前模式是否低于规则要求的最低模式（min_mode）"""
    cur_level = MODE_LEVEL_ORDER.get((current_mode or "").lower(), 0)
    req_level = MODE_LEVEL_ORDER.get((required_mode or "").lower(), 99)
...
def load_perm_path_config(root_dir: str, username: str, log_error_func) -> bool:
    ...
    try:
        ...
        for path_pattern, rule_value in raw_config.items():
            ...
            # 新格式：dict {mode, allow_advanced_syntax, commands, min_mode}
            if isinstance(rule_value, dict):
                ...
                min_mode = rule_value.get("min_mode")
                # 校验 min_mode：只接受 low/mid/adv，非法值忽略（不限制）
                if min_mode is not None:
                    min_mode = str(min_mode).strip().lower()
                    if min_mode not in MODE_LEVEL_ORDER:
                        _debug_print(f"忽略无效 min_mode：{min_mode}（规则 {path_pattern}）")
                        min_mode = None
            # 旧格式兼容：纯 list
...
def _parse_single_path_pattern(path_pattern: str, allowed_cmds: List[str],
                                ...
                                log_error_func,
                                mode: str = "whitelist",
                                allow_advanced_syntax: bool = True,
                                min_mode: str = None) -> Optional[Dict]:
    """
    解析单个路径模式
    ...
    # 构建规则：固定前缀中的 <username> 替换为真实用户名，物理路径才能命中 home 规则
    fixed_phys_raw = fixed.replace('<username>', username)
    rule = {
        'fixed_virt': fixed,
        'fixed_phys': _resolve_virt_to_phys(fixed_phys_raw, root_dir),
        'name_pattern': name_pattern,
        'depth': depth,
        'allowed': [cmd.lower() for cmd in allowed_cmds],
        'raw_pattern': path_pattern,
...
def check_path_permission_for_cmd(cmd_head: str, all_phys_paths: List[str], 
    ...
    - AI 调用: 保持严格拦截
    - 人类调用: home 与其它路径一样走细颗粒规则，low/mid 弹确认，adv 弹确认
    ...
    for phys_path in all_phys_paths:
        ...
        if is_hit and matched_rule:
            ...
            rule_min = matched_rule.get('min_mode')
            if rule_mode == 'blacklist':
                ...
                if cmd_lower in matched_rule['allowed']:
                    ...
                    if rule_min and _mode_below(current_mode, rule_min) and min_mode_violation is None:
                        min_mode_violation = (phys_path, rule_min)
            else:
    ...
    # 规则设置了 min_mode 且当前模式低于它 → 直接拒绝，不弹确认框
    if min_mode_violation:
        path_v, required_mode = min_mode_violation
        deny_msg = _get_msg("min_mode_denied", cmd=cmd_head, path=path_v,
                            required=required_mode, mode=current_mode)
        print(Fore.RED + deny_msg + Style.RESET_ALL)
        if log_error_func:
            log_error_func(f"权限不足拦截：命令 {cmd_head} 在路径 {path_v} 上需要 {required_mode} 及以上模式（当前 {current_mode}）", request_id)
        return False
...
def check_fine_grained_advanced_syntax(phys_paths: List[str], root_dir: str, username: str,
    ...
    # 规则设置了 min_mode 且当前模式低于它 → 直接拒绝，不弹确认框
    if hit_rule and hit_rule.get('min_mode') and _mode_below(current_mode, hit_rule['min_mode']):
        required_mode = hit_rule['min_mode']
        deny_msg = _get_msg("min_mode_denied_adv_syntax", path=hit_path,
                            required=required_mode, mode=current_mode)
        print(Fore.RED + deny_msg + Style.RESET_ALL)
        if log_error_func:
            log_error_func(f"权限不足拦截：{current_mode}模式在路径 {hit_path} 上使用高级语法，需要 {required_mode} 及以上模式", request_id)
        return False
...
def is_command_allowed_in_mode(cmd_name: str, mode: str, global_config: dict, 
    ...
    try:
        ...
        # 2026-09 加固（M1）：原 len<=2 放行让 `sh` 这类 2 字符命令绕过白名单，
        # 收紧为仅 len==1。
        if len(cmd_name) <= 1:
            return True
        ...
        # 2026-09 加固（M1）：原异常时 return True（放行）→ fail-open，
        # 改为 fail-closed（拦截），避免配置/解析异常导致白名单失效
        if log_error_func:
...
def _normalize_danger_str(s: str) -> str:
    """高危命令/模式归一化：去掉所有空白。

    2026-09 安全加固：仓库内 dan_cmd 文件存在空格缺失的损坏条目
    （如 `rm-rf /home`），纯子串匹配会让 `rm -rf /home` 永远不命中。
    匹配前统一移除空白，损坏条目与真实命令即可正确匹配，
    同时天然容忍用户输入 `rm  -rf  /` 之类的变体间距。
    """
...
def _load_dan_cmd_patterns() -> Optional[List[str]]:
    ...
    try:
        ...
        with open(dan_cmd_path, "r", encoding="utf-8") as f:
            # 加载时即做空白归一化（_normalize_danger_str），保证匹配一致性
            patterns = [_normalize_danger_str(line) for line in f
...
def check_dangerous_commands(cmd_str: str, user_mode, log_info_func=None, 
    ...
    # 默认的高危命令模式（同样按 _normalize_danger_str 归一化后匹配）
    default_patterns = [
    ...
    blocked_patterns = [_normalize_danger_str(p) for p in default_patterns]
    cached_patterns = _load_dan_cmd_patterns()
    if cached_patterns is not None:
        blocked_patterns = cached_patterns

    # 归一化命令串（去空白），与归一化后的模式做子串匹配——
    # 兼容损坏条目（rm-rf /home）与真实命令（rm -rf /home）间的间距差异
    cmd_lower = _normalize_danger_str(cmd_str)
    ...
    for pattern in blocked_patterns:
        if pattern in cmd_lower:
            if current_mode == "adv":
                # 【修复1】：先检查全局开关
                cmd_type = 'dangerous'
                # 用原始命令分词判断命令头是否为 rm（cmd_lower 已归一化去空格，不能 split）
                _orig_parts = (cmd_str or "").lower().split()
                if _orig_parts and 'rm' in _orig_parts[0]:
                    cmd_type = 'rm'
