...
# ── 2026-09 安全加固：cron 执行前破坏性命令门 ──
# 定时任务以 shell 形式执行持久化 prompt，一旦存储被篡改或被提示词注入的 AI
# 注册恶意任务，到点即执行任意命令。这里在调度执行前拦截明显破坏性模式
# （与 dan_cmd 一致的空白归一化子串匹配）。真正的创建端防线见
# ai_cmd.py CronCreate（DangerFullAccess，需用户显式批准）。
_CRON_BLOCKED_PATTERNS = [
    "rm-rf/", "rm-r/", "rmdir/", "rd/",
    "mkfs", "fdisk", "parted", "wipefs", "shred", "blkdiscard",
    "ddif=/dev/zero", "ddif=/dev/urandom", "ddoof=/dev/sd",
    "chmod-r777", "chmod-r000", "chown-r",
    "shutdown", "reboot", "poweroff", "halt", "init0", "init6",
    "sudorm", "sudodd", "sudomkfs", "sudofdisk",
    ":(){", "forkbomb", "killall-9", "pkill-9",
    ">/dev/sda", ">/dev/sdb", "/etc/passwd", "/etc/shadow",
    "sysrq-trigger", "sysctl-w", "echo1>/proc",
]


def _cron_prompt_blocked(prompt: str) -> Optional[str]:
    """检查 cron prompt 是否命中破坏性模式。命中返回命中的模式，否则 None。"""
    if not prompt:
        return None
    norm = "".join(prompt.lower().split())
    for pattern in _CRON_BLOCKED_PATTERNS:
        if pattern in norm:
            return pattern
    return None
...
def _execute_cron_prompt(prompt: str, cron_id: str) -> None:
    ...
    2026-09 加固：执行前先过破坏性命令门，命中即拒绝执行并记录日志。
    """
    try:
        blocked = _cron_prompt_blocked(prompt)
        if blocked:
            logging.error("Cron %s: 破坏性命令门拦截，拒绝执行 — 命中模式 %r，prompt=%r",
                          cron_id, blocked, prompt[:200])
            return
