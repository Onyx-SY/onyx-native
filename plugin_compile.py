...
Usage:
  python plugin_compile.py source.c                  # compile to ~/.ai_onyx_plugin/
  python plugin_compile.py source.c -o output_name   # custom name
  python plugin_compile.py --list                    # list installed plugins

For full documentation see plugin_loader.py
"""

import os, sys, platform, subprocess, argparse, shutil

PLUGIN_DIR = os.path.join(os.path.expanduser("~"), ".ai_onyx_plugin")
LIB_C_CODE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lib", "c_code")


def _sys() -> str:
    s = platform.system().lower()
    return "termux" if ("termux" in sys.prefix.lower()) else s
...
def list_plugins() -> list:
    if not os.path.isdir(PLUGIN_DIR):
        return []
    return [f for f in sorted(os.listdir(PLUGIN_DIR)) if f.endswith((".so", ".dll", ".dylib"))]
...
def main():
    ...
    if not a.source:
        print("Usage: python plugin_compile.py source.c [-o name]")
        return
